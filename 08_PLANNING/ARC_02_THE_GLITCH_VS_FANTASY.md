# Arc 2 - The Glitch vs. Fantasy

## Authority and status

- source authority: 08_PLANNING/MASTER_STORY_PLAN.md
- parent volume/book: UNKNOWN / NOT ESTABLISHED
- arc status: APPROVED ROADMAP / ARC PLAN TO BE REFINED
- chapter count and range: UNKNOWN / NOT ESTABLISHED
- chapter titles: UNKNOWN / NOT ESTABLISHED

This boundary file records only the Arc 2 material established by the master plan. It does not create companions, quests, locations, or chapter events.

## Purpose

- establish the protagonist's frightening combat style;
- develop genuine companions and relationships;
- expand the fantasy world;
- show real growth that the System cannot measure.

## Entry state

- prior arc exit: the protagonist has survived an event that should have killed a beginner; exact event and consequences UNKNOWN / NOT ESTABLISHED;
- location: UNKNOWN / NOT ESTABLISHED;
- protagonist identity: still publicly using Deadpool while identity uncertainty remains unresolved;
- existence state: unregistered System anomaly;
- defining abilities: abnormal regeneration and the already-active unrestricted imagination anomaly;
- System progression: no level, class, or normal progression sheet;
- fourth-wall state: full awareness and narrative-editing authority carried from the opening;
- companions: UNKNOWN / NOT ESTABLISHED;
- active objective: accept and complete simple quests; exact quest list UNKNOWN / NOT ESTABLISHED.

## Protected rules

- His actual improvement comes from experience, strategy, reflexes, timing, weapon proficiency, magic understanding, environmental awareness, pain tolerance, regeneration control, and related learning.
- None of that improvement appears as normal System levels or status values.
- Regeneration supports dangerous tactics but does not solve imprisonment, sealing, paralysis, soul damage, curses, dimensional exile, memory interference, conceptual attacks, or existence attacks.
- The world itself remains consequential; comedy is a coping mechanism, not proof that suffering is meaningless.
- A future relationship must be earned through events; no companion is established by this roadmap file.
- The imagination anomaly has no mechanical counter or blocker; Arc 2 tension comes from Red Jackal's choices, consequences, relationships, and understanding of the world.

## Known story movement

1. He accepts simple quests.
2. The quests become more dangerous or complicated than expected.
3. He uses suicidal tactics that sane adventurers would avoid.
4. His reputation grows: some call him fearless, cursed, immortal, or insane.
5. He begins forming genuine relationships that can separate his chosen identity from Deadpool.
6. He becomes more capable without receiving measurable System progression.

Exact quest content, companions, faction names, locations, injuries, and chapter order are UNKNOWN / NOT ESTABLISHED.

## Power and fourth-wall boundary

- allowed growth: non-System mastery, combat creativity, tactical thinking, regeneration control, and deliberate or reluctant use of an already-unrestricted imagination anomaly;
- permanent Evolution: UNKNOWN / NOT ESTABLISHED; do not add one without a parent authority and earned prerequisites;
- System status: remains UNDEFINED and cannot become a normal progression sheet;
- fourth-wall stage: no stage progression; full supernatural narrative authority is already authorized;
- future counters: restraint, sealing, imprisonment, and other non-ordinary attacks remain valid regeneration and world-stakes pressures, but they cannot counter the underlying imagination anomaly.

## Story memory

- relevant mystery IDs: UNKNOWN / NOT ESTABLISHED;
- relevant foreshadowing IDs: UNKNOWN / NOT ESTABLISHED;
- permitted movement: plant or reinforce identity and System mysteries through lived consequences;
- forbidden reveal: the origin of the protagonist's registration failure;
- ledger updates require approved prose references.

## Arc ending gate

He defeats something far beyond what a supposedly unranked adventurer should survive. Rumors spread.

The opponent, location, witnesses, method, cost, and exact consequences are UNKNOWN / NOT ESTABLISHED.

## Chapter dependency map

- chapter count: UNKNOWN / NOT ESTABLISHED;
- chapter titles: UNKNOWN / NOT ESTABLISHED;
- inherited entry states: UNKNOWN / NOT ESTABLISHED;
- permitted chapter deltas: UNKNOWN / NOT ESTABLISHED;
- exit-state handoff: UNKNOWN / NOT ESTABLISHED.

## Exit state

- location and timeline: UNKNOWN / NOT ESTABLISHED;
- companions and relationships: UNKNOWN / NOT ESTABLISHED;
- non-System skills and mastery: exact changes UNKNOWN / NOT ESTABLISHED;
- System status: still UNDEFINED;
- identity movement: relationships begin to provide an identity separate from imitation;
- next progression boundary: Arc 3 may escalate external attention and restore memory fragments after an approved entry state exists.
