# Arc 1 - Red Jackal Tells You: The Man Who Thinks He's Deadpool

## Authority and status

- source authority: 08_PLANNING/MASTER_STORY_PLAN.md
- parent volume/book: UNKNOWN / NOT ESTABLISHED
- arc status: APPROVED ROADMAP / ARC PLAN TO BE REFINED
- chapter count and range: UNKNOWN / NOT ESTABLISHED
- chapter titles: UNKNOWN / NOT ESTABLISHED

This boundary file records only the Arc 1 material established by the master plan. It is not numbered prose and does not establish missing cast, location, or chapter facts.

## Purpose

- introduce the amnesiac protagonist;
- establish the comedy and dark-consequence balance;
- introduce the fantasy world and System rules;
- introduce the adventurer guild as a world institution;
- begin the identity mystery.

## Entry state

- timeline/order position: opening arc; exact position UNKNOWN / NOT ESTABLISHED;
- location: divine summoning chamber at opening; later mortal-world location UNKNOWN / NOT ESTABLISHED;
- protagonist identity: Red Jackal, an amnesiac Earth-born human who believes he is Deadpool because those memories survived more clearly than his own;
- protagonist existence state: damaged and unregistered by the System; exact soul state UNKNOWN / NOT ESTABLISHED;
- defining abilities: abnormal regeneration and an unrestricted imagination anomaly caused by his foreign-code existence;
- System level/class/title: UNDEFINED;
- fourth-wall state: full awareness and narrative-editing authority from the opening;
- companions and relationships: UNKNOWN / NOT ESTABLISHED;
- active objective: reach and survive the mortal world; exact objective after arrival UNKNOWN / NOT ESTABLISHED.

## Protected rules

- He is never secretly Wade Wilson.
- The System does not give him a normal level, class, title, or progression sheet.
- The official System terminology remains UNDEFINED, NO RECORD, FAILED, UNAVAILABLE, and ANOMALY.
- Regeneration prevents ordinary death but does not remove pain, exhaustion, fear, restraint, sealing, soul damage, curses, or other counters.
- His audience-facing comments and narrative edits are real from the opening; they are not a later unlock.
- Red Jackal's imagination anomaly is already active, can make anything he imagines happen, and has no cooldown, cost, focus requirement, understanding requirement, range restriction, counter, suppression method, exhaustion rule, or known upper limit.
- Any choice not to use the anomaly must be character-driven; it cannot be explained as inability.

## Known story movement

1. He awakens with severe memory loss.
2. His Deadpool memories remain unusually clear.
3. His regeneration convinces him that he must actually be Deadpool.
4. A goddess rejects that identity; he refuses to accept the explanation.
5. He reaches the mortal world and encounters an adventurer guild.
6. Guild examinations repeatedly return UNDEFINED through different examiners or equipment.
7. He adopts Deadpool as his name despite the unresolved identity question.
8. He receives the lowest practical adventurer classification because the guild cannot classify him normally.

Exact goddess identity, guild name, mortal settlement, examiner identities, and chapter events are UNKNOWN / NOT ESTABLISHED.

## Power and fourth-wall boundary

- allowed growth: survival, observation, basic combat experience, practical adaptation, and learning how his already-unrestricted anomaly affects his choices;
- System progression: no level or normal class progression;
- regeneration: present, powerful, painful, and limited by non-ordinary counters;
- fourth-wall stage: Full from the opening;
- forbidden early reveal: none for fourth-wall awareness; the reader must understand that Red Jackal can address the actual audience and alter the scene from the opening.

## Story memory

- relevant mystery IDs: UNKNOWN / NOT ESTABLISHED;
- relevant foreshadowing IDs: UNKNOWN / NOT ESTABLISHED;
- permitted movement: introduce identity, System, and registration mysteries without resolving them;
- forbidden reveal: the reason the soul is unregistered;
- ledgers must be updated only after approved prose canonizes a seed or clue.

## Arc ending gate

The protagonist survives something that should unquestionably kill a beginner. Witnesses begin questioning whether he is truly abnormal.

The exact event, witnesses, location, injuries, and consequences are UNKNOWN / NOT ESTABLISHED.

## Chapter dependency map

- chapter count: UNKNOWN / NOT ESTABLISHED;
- chapter titles: UNKNOWN / NOT ESTABLISHED;
- inherited entry states: UNKNOWN / NOT ESTABLISHED;
- permitted chapter deltas: UNKNOWN / NOT ESTABLISHED;
- exit-state handoff: UNKNOWN / NOT ESTABLISHED.

## Exit state

- location and timeline: UNKNOWN / NOT ESTABLISHED;
- companions and relationships: UNKNOWN / NOT ESTABLISHED;
- knowledge changes: the protagonist and witnesses may have different interpretations of his abnormality;
- System status: still undefined by the master-plan rule;
- mystery status: unresolved;
- next progression boundary: Arc 2 may develop combat style, relationships, world knowledge, and non-System growth only after an approved entry state exists.
